...
public class PackageExplorerWidget extends DirtyableComposite {
	...
	public PackageExplorerWidget(EditItemEvent edit, String uuid,
		...
		treeListener = new TreeListener() {
			...
			public void onTreeItemSelected(TreeItem selected) {
				PackageTreeItem event = (PackageTreeItem) selected
						.getUserObject();
		...
		if (snapshotName == null) {
			...
			buttons.getCellFormatter().setAlignment(0, 0,
		...
		layout
				.setWidget(
						...
						1,
						new HTML(
								"<i>Please choose a package to edit, explore, or create a new package.</i>"));
	...
	private HorizontalPanel getNewWizardButtons() {
		...
		uploadModel
				.setTitle("This creates a new model archive - models contain classes/types that rules use.");
	...
	private void addPackage(final PackageConfigData conf, boolean preSelect) {
		...
		TreeItem pkg = makeItem(conf.name, "images/package.gif",
		...
		pkg.addItem(makeItem("Technical rule assets",
				"images/technical_rule_assets.gif", showListEvent(conf.uuid,
						AssetFormats.TECHNICAL_RULE_FORMATS)));
		pkg
				.addItem(makeItem("Functions", "images/function_assets.gif",
						showListEvent(conf.uuid,
								new String[] { AssetFormats.FUNCTION })));
		pkg.addItem(makeItem("DSL", "images/dsl.gif", showListEvent(conf.uuid,
				new String[] { AssetFormats.DSL })));
		pkg.addItem(makeItem("Model", "images/model_asset.gif", showListEvent(
				conf.uuid, new String[] { AssetFormats.MODEL })));
		...
		RepositoryServiceFactory.getService().loadPackageConfig(uuid,
				new GenericCallback() {
					...
					public void onSuccess(Object data) {
						...
						FormStyleLayout infoLayout = new FormStyleLayout(
								"images/package_large.png", conf.name);
						infoLayout.setStyleName("package-Editor");
						infoLayout.setWidth("100%");
						infoLayout.addAttribute("Description:", new Label(
								conf.description));
						infoLayout.addAttribute("Date created:", new Label(
								conf.dateCreated.toLocaleString()));

						if (conf.isSnapshot) {
							infoLayout.addAttribute("Snapshot created on:",
									new Label(conf.lastModified
											.toLocaleString()));
							infoLayout.addAttribute("Snapshot comment:",
									new Label(conf.checkinComment));
							final String uri = PackageBuilderWidget
									.getDownloadLink(conf);
							...
							infoLayout.addAttribute("Package URI:", new Label(
									uri));
							Button viewSource = new Button(
									"View package source");
							viewSource.addClickListener(new ClickListener() {
								public void onClick(Widget w) {
									PackageBuilderWidget.doBuildSource(
											conf.uuid, conf.name);
								}
							});
