...
public class ImportsWidget
    ...
    private final ListBox importList = new ListBox(true);

    public ImportsWidget(Path path) {
        this(path, false);
    }

    public ImportsWidget(Path path, boolean readOnly) {
        this.path = path;
                            ...
                            if (importList.getSelectedIndex() > -1) {
                                for (int i = 0; i < importList.getItemCount(); i++) {
                                    if (importList.isItemSelected(i)) {
                                        ...
                                        ImportsWidget.this.imports.remove(i);
                                        i--;
                                    }
                                }
                            }
    ...
    private void showTypeQuestion(final String headerMessage) {
        ...
        }, DataModelService.class).getFactTypes(path);
