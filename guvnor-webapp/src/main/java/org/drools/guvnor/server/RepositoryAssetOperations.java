...
import org.drools.repository.SortDirection;
import org.drools.repository.SortableColumnMetaData;
...
public class RepositoryAssetOperations {
    ...
    private AssetItemIterator getAssetIterator( AssetPageRequest request ) {
        ...
        if ( request.getFormatInList() != null ) {
            ...
            iterator = packageItem.listAssetsByFormat( request.getFormatInList(),
                                                       convertSortableColumnsMetaData( request.getSortMetaData() ) );
    ...
    private SortableColumnsMetaData convertSortableColumnsMetaData( final org.drools.guvnor.client.rpc.SortableColumnsMetaData columnsMetaData ) {
        final SortableColumnsMetaData rMetaData = new SortableColumnsMetaData();
        if ( columnsMetaData == null ) {
            return rMetaData;
        }
        for ( org.drools.guvnor.client.rpc.SortableColumnMetaData cmd : columnsMetaData.getSortListOrder() ) {
            rMetaData.getSortListOrder().add( convertSortableColumnMetaData( cmd ) );
        }
        return rMetaData;
    }

    private SortableColumnMetaData convertSortableColumnMetaData( final org.drools.guvnor.client.rpc.SortableColumnMetaData columnMetaData ) {
        final SortableColumnMetaData rMetaData = new SortableColumnMetaData( columnMetaData.getColumnName(),
                                                                             convertSortableColumnMetaDataSortDirection( columnMetaData.getSortDirection() ) );
        return rMetaData;
    }

    private SortDirection convertSortableColumnMetaDataSortDirection( final org.drools.guvnor.client.widgets.tables.sorting.SortDirection direction ) {
        switch ( direction ) {
            case ASCENDING:
