...
public class PackageDeploymentServlet extends RepositoryServlet {
    ...
    protected void doHead(HttpServletRequest request,
        ...
        if ( request.getMethod().equals( "HEAD" ) ) {
            SimpleDateFormat dateFormat = new SimpleDateFormat( RFC822DATEFORMAT,
                                                                HEADER_LOCALE );
    ...
    /**
     * Get the binary package. This will get the compiled package stuff from
     * either the latest package, or a snapshot.
     * 
     * The end of the URI is of the form: /<packageName>/(<snapshotVersionName>
     * | LATEST)
     * 
     * if you pass in "LATEST" it will get the latest (not a snapshot) if it
     * exists. Normally that will only be used when downloading on demand,
     * otherwise you should ONLY use a snapshot as they are always "up to date".
     */
                                    ...
                                    if ( helper.isSource() ) {
                                        ...
                                        if ( req.getRequestURI().endsWith( "SCENARIOS" ) ) {
                                            ...
                                            if ( numberOfAssets > 1 ) {
                                                ...
                                                while ( (count = inputFile.read( data,
                                                                                 0,
                                                                                 1000 )) != -1 )
                             {
                                 out.write( data,
    ...
    public InputStream zipModel(PackageItem pkg) {
        ...
        if ( jarAssets.size() != 0 ) {
            assetZipper = new AssetZipper( jarAssets,
                                           pkg );
