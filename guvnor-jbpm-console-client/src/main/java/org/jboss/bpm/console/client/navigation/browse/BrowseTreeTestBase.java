...
public class BrowseTreeTestBase {
    ...
    private void setUpView() {
        view = Mockito.mock(BrowseTreeView.class);
        find = Mockito.mock(IsTreeItem.class);
        rootTreeItem = Mockito.mock(IsTreeItem.class);
        rootCategoryTreeItem = Mockito.mock(IsTreeItem.class);
        IsTreeItem rootStatesTreeItem = Mockito.mock(IsTreeItem.class);
        incomingInboxTreeItem = Mockito.mock(IsTreeItem.class);
        inboxRecentEdited = Mockito.mock(IsTreeItem.class);
        inboxRecentViewed = Mockito.mock(IsTreeItem.class);
        Mockito.when(view.addFind()).thenReturn( find );
        Mockito.when(view.addRootTreeItem()).thenReturn( rootTreeItem );
        Mockito.when(view.addRootCategoryTreeItem()).thenReturn( rootCategoryTreeItem );
        Mockito.when(view.addRootStateTreeItem()).thenReturn(rootStatesTreeItem);
        Mockito.when(view.addInboxIncomingTreeItem()).thenReturn( incomingInboxTreeItem );
        Mockito.when(view.addInboxRecentEditedTreeItem()).thenReturn( inboxRecentEdited );
        Mockito.when(view.addInboxRecentViewedTreeItem()).thenReturn( inboxRecentViewed );
    }
    ...
    protected void setUpChildren(IsTreeItem parent, IsTreeItem... children) {
        ...
        Mockito.when(view.getChildren(parent)).thenReturn( rootChildList );
    }
    ...
    protected void verifyAddedTreeItemsToCategory(IsTreeItem parent, String... categories) {
        for (String category : categories) {
            Mockito.verify(view).addTreeItem( parent, category );
        }
    }
