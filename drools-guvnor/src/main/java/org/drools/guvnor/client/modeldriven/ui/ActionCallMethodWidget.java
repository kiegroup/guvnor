...
public class ActionCallMethodWidget extends DirtyableComposite {
       ...
       public ActionCallMethodWidget(RuleModeller mod, ActionCallMethod set,
           ...
           if (completions.isGlobalVariable(set.variable)) {
               this.fieldCompletions = completions
                       .getFieldCompletionsForGlobalVariable(set.variable);
               this.variableClass = (String) completions.globalTypes
                       .get(set.variable);
           } else {
               FactPattern pattern = mod.getModel().getBoundFact(set.variable);
               if (pattern !=null){
                   List<String> methodList = completions
                           .getMethodNames(pattern.factType);
                   ...
                   if (patternRhs!=null){
                       List<String> methodList = completions
                           .getMethodNames(patternRhs.factType);
                       fieldCompletions = new String[methodList.size()];
                       int i = 0;
                       for (String methodName : methodList) {
                           fieldCompletions[i] = methodName;
       ...
       private Widget getSetterLabel() {
           ...
           if (model.state == ActionCallMethod.TYPE_UNDEFINED) {
               ...
               horiz.add(new SmallLabel(HumanReadable.getActionDisplayName("call")
                       + " [" + model.variable + "]")); // NON-NLS
               horiz.add(edit);
           } else {
               horiz.add(new SmallLabel(HumanReadable.getActionDisplayName("call")
                       + " [" + model.variable + "."+model.methodName+"]")); // NON-NLS
           }
           return horiz;
       }
       ...
       protected void showAddFieldPopup(Widget w) {
           ...
           box.addChangeListener(new ChangeListener() {
               public void onChange(Widget w) {
                   ...
                   FactPattern pattern = modeller.getModel().getBoundFact(
                           model.variable);
                   if (pattern!= null){
                       fieldList = completions.getMethodFields(
                               pattern.factType, methodName);
                   }
                   else {
                       ActionInsertFact patternRhs = modeller.getModel().getRhsBoundFact(model.variable);
                       if (patternRhs!=null){
                           fieldList = completions.getMethodFields(
                                   patternRhs.factType, methodName);
                       }
                   }
       ...
       private Widget valueEditor(final ActionFieldFunction val) {
           ...
           if (this.completions.isGlobalVariable(this.model.variable)) {
               type = (String) this.completions.globalTypes
                       .get(this.model.variable);
           } else {
           ...
           DropDownData enums = this.completions.getEnums(type,
                   this.model.fieldValues, val.field);
           return new MethodParameterValueEditor(val, enums, modeller,
                   val.type);
       }
