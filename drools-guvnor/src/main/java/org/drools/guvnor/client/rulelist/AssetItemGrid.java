...
/**
 * Uses an awesome grid and does paging for asset lists. It works well, but here
 * be dragons.
 * 
...
public class AssetItemGrid extends Composite {
    ...
    /**
     * Call this to set up a table config instead of loading it from the server.
     * You then pass in the config name for later use. Can save a round trip.
     */
    ...
    /**
     * Create a grid using the given config - config will be loaded from the
     * server if it is not already cached. You can use registerTableConfig to
     * register it to avoid a server hit.
     */
    ...
    /**
     * Similar to the other constructor, but takes an optional feedURL to show
     * with an atom icon in the top right.
     */
    public AssetItemGrid(final OpenItemCommand event,
                         ...
                         new GenericCallback<TableDataResult>() {
                             public void onSuccess(TableDataResult result) {
                                 ...
                                 for ( int i = 0; i < result.data.length; i++ ) {
                                     ...
                                     for ( int j = 2; j < numFlds; j++ ) {
                                         if ( rd.getFields()[j] instanceof DateFieldDef ) {
                                             ...
                                             // NOTE, GWTEXT only understands
                                             // certain date formats, for
                                             // example "yyyy/MM/dd"
                                             // works but other formats such as
                                             // "yyyy/MM/dd" or localized
                                             // formats is not recognizable
                                             // by GWTEXT. See
                                             // http://code.google.com/p/gwt-ext/issues/detail?id=459&start=100.
                                 ...
                                 store = new Store( proxy,
                                                    reader );
                                 // currentGrid = new Grid(Ext.generateId(),
                                 // "600px", "600px", store, cm);
    ...
    private static RecordDef createRecordDef(TableConfig conf) {
        FieldDef[] fd = new FieldDef[conf.headers.length + 2]; // 2 as we have
                                                               // format and
                                                               // UUID to tack
                                                               // on.
        ...
        for ( int i = 0; i < conf.headers.length; i++ ) {
            ...
            cfgs[i + 1] = new ColumnConfig() {
                ...
                private void handleDescription() {
                    setHidden( true ); // don't want to show a separate
                                       // description
                }
                ...
                /**
                 * This dirty hack is needed to cope with keys that may not be
                 * localised. ie not built in ones (eg a user adds a custom
                 * column).
                 */
