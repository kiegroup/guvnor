...
public class RepositoryVisitor {
        ...
        try {
            ...
            if ( oldIdentityHash != null ) {
                ...
                for ( String path : deletedFiles.keySet() ) {
                    boolean deleted = new File( root.getAbsolutePath().replace( projectPath.toString(), "" ) + "/" + path ).delete();
                    System.out.println( "Deleted: " + root.getAbsolutePath().replace( projectPath.toString(), "" ) + "/" + path + " -> " + deleted );
                }
            }
    ...
    private void loadIndex( String path ) {
        ...
        try {
            ...
            oldIdentityHash = gson.fromJson( reader, type );
     ...
     */
    private void readFile( final BufferedInputStream origin, byte[] data, final int BUFFER ) throws IOException {
        while ( ( origin.read( data, 0, BUFFER ) ) != -1 ) {
        }
    }

    private FileOutputStream writeFile( final File parent, final Path path, FileOutputStream output, final BufferedInputStream origin, byte[] data, final int BUFFER ) throws IOException, FileNotFoundException {
