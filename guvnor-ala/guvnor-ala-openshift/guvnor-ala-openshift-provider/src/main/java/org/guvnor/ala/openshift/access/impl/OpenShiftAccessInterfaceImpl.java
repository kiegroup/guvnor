...
public class OpenShiftAccessInterfaceImpl implements OpenShiftAccessInterface, Disposable {
    ...
    static OpenShiftConfig buildClientConfig(OpenShiftProviderConfig config) {
        ...
        /* NOTE: only respected via system property or environment variable
         *  SEE: io.fabric8.kubernetes.client.Config.tryKubeConfig(Config config)
        String kubeconfig = trimToNull(config.getKubeconfig());
        if (kubeconfig != null) {
            //builder = builder.with?(kubeconfig);
        }
         */
        /* NOTE: only respected via system property or environment variable
         *  SEE: io.fabric8.kubernetes.client.Config.tryNamespaceFromPath(Config config)
        String kubenamespace = trimToNull(config.getKubenamespace());
        if (kubenamespace != null) {
            builder = builder.with?(kubenamespace);
        }
         */
        ...
        /* NOTE: only respected via system property or environment variable
         *  SEE: io.fabric8.kubernetes.client.Config.tryKubeConfig(Config config)
        String kubernetesAuthTryKubeConfig = trimToNull(config.getKubernetesAuthTryKubeConfig());
        if (kubernetesAuthTryKubeConfig != null) {
            //builder = builder.with?(kubernetesAuthTryKubeConfig);
        }
         */
        /* NOTE: only respected via system property or environment variable
         *  SEE: io.fabric8.kubernetes.client.Config.tryServiceAccount(Config config)
        String kubernetesAuthTryServiceAccount = trimToNull(config.getKubernetesAuthTryServiceAccount());
        if (kubernetesAuthTryServiceAccount != null) {
            builder = builder.with?(kubernetesAuthTryServiceAccount);
        }
         */
        ...
        /* NOTE: only respected via system property or environment variable
         *  SEE: io.fabric8.kubernetes.client.Config.tryNamespaceFromPath(Config config)
        String kubernetesTryNamespacePath = trimToNull(config.getKubernetesTryNamespacePath());
        if (kubernetesTryNamespacePath != null) {
            builder = builder.with?(kubernetesTryNamespacePath);
        }
         */
